      ...
      <div style={{ maxWidth: '840px', margin: '0 auto' }}>
        ...
        <div style={{ display: 'grid', gap: '28px' }}>
          <GlassPanel padding="32px">
            <h2 style={{ fontSize: '20px', fontWeight: 600, marginBottom: '24px', color: colors.accent }}>
              {t('docs.userGuide')}
            </h2>

            {steps.map(({ title, body }) => (
              <div key={title} style={{ marginBottom: '20px' }}>
                <h3 style={{ fontSize: '16px', fontWeight: 600, marginBottom: '8px', color: colors.text1 }}>{title}</h3>
