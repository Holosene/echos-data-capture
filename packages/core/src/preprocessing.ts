...
/**
 * Apply CLAHE to improve local contrast without amplifying noise.
 * Divides the image into tiles, equalizes each tile's histogram with a clip limit,
 * then bilinearly interpolates between tiles for smooth transitions.
 */
function clahe(
  pixels: Float32Array,
  w: number,
    ...
    for (let tx = 0; tx < tilesX; tx++) {
      ...
      for (let y = startY; y < endY; y++) {
        for (let x = startX; x < endX; x++) {
          const bin = Math.min(BINS - 1, (pixels[y * w + x] * (BINS - 1)) | 0);
          hist[bin]++;
        }
      }

      // Clip histogram and redistribute
      const clipCount = clipLimit * tilePixels / BINS;
      let excess = 0;
  ...
  for (let y = 0; y < h; y++) {
    ...
    for (let x = 0; x < w; x++) {
      ...
      const top = v00 + (v10 - v00) * Math.max(0, Math.min(1, fx));
      const bot = v01 + (v11 - v01) * Math.max(0, Math.min(1, fx));
      out[y * w + x] = top + (bot - top) * Math.max(0, Math.min(1, fy));
    }
  }
...
// ─── Unsharp Mask (edge-preserving sharpening) ───────────────────────────────

/**
 * Sharpen details using unsharp masking: original + amount * (original - blurred).
 * Uses the existing Gaussian blur as the low-pass filter.
 */
function unsharpMask(
  ...
  if (amount <= 0 || sigma <= 0) return pixels;

  const blurred = gaussianBlur(pixels, w, h, sigma);
  const out = new Float32Array(w * h);
  for (let i = 0; i < out.length; i++) {
    out[i] = clamp(pixels[i] + amount * (pixels[i] - blurred[i]), 0, 1);
  }
  return out;
}
...
export function preprocessFrame(
  ...
  // 7. CLAHE — local contrast enhancement (reveals subtle structures)
  const claheClip = settings.claheClipLimit ?? 0;
  if (claheClip > 0) {
    intensity = clahe(intensity, w, h, claheClip, 8, 8);
  }

  // 8. Unsharp mask — sharpen edges after denoising
  const sharpenAmt = settings.sharpenAmount ?? 0;
  const sharpenSig = settings.sharpenSigma ?? 1.5;
  if (sharpenAmt > 0) {
    intensity = unsharpMask(intensity, w, h, sharpenSig, sharpenAmt);
  }
